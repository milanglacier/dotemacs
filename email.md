...
- [Tools I used for my workflow](#tools-i-used-for-my-workflow)
- [Syncing Notmuch Tags with IMAP Folders: Workarounds for Cross-Device
  Email
  Management](#syncing-notmuch-tags-with-imap-folders-workarounds-for-cross-device-email-management)
  - [The shell script to move mail between Maildir
    folders](#the-shell-script-to-move-mail-between-maildir-folders)
  - [Notmuch config to incorporate the above movemail
    script](#notmuch-config-to-incorporate-the-above-movemail-script)
    - [Example pre-new script](#example-pre-new-script)
    - [Example post-new script](#example-post-new-script)
    - [Explanation](#explanation)
    - [Example `$HOME/.notmuch-config`
      File](#example-home.notmuch-config-file)
  - [Emacs configuration](#emacs-configuration)
  - [Full Command for Syncing Emails](#full-command-for-syncing-emails)
- [Configure oauth2 support with
  outlook](#configure-oauth2-support-with-outlook)
  - [Use the mutt script for oauth2
    authentication](#use-the-mutt-script-for-oauth2-authentication)
  - [Set up app registration in
    Azure](#set-up-app-registration-in-azure)
  - [Fetch the authentication token with the mutt
    script](#fetch-the-authentication-token-with-the-mutt-script)
  - [Configure mbsync](#configure-mbsync)
  - [Configure msmtp](#configure-msmtp)
- [From `mu4e` to `notmuch`](#from-mu4e-to-notmuch)
...
The
[mutt<sub>oauth2</sub>.py](https://github.com/neomutt/neomutt/tree/main/contrib/oauth2)
script, obtained from NeoMutt's contributions, can be used for OAuth2
authentication. Earlier version of this script required manual
modification—such as changing the GPG recipient or token encryption
method—but the latest version now accepts these settings as command-line
arguments. This eliminates the need for direct script modifications,
allowing for direct use.
...
3.  Follow the
    ...
    `Manage -> Authentication` and `Manage -> API permissions` sidebars
    in your app portal. It's crucial to enable access for IMAP, SMTP,
    POP, and other relevant scopes as detailed in the mutt script under
    the `Manage -> API permissions` sidebar.
...
encrypt the OAuth2 token and save it to `your/path/to/the/oauth/file`.
